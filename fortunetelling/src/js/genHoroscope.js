...
const get_days_from_start = function() {
    const date = new Date();
    ...
    const daysFromStart = Math.floor((currentDate - startingDate) / (1000 * 60 * 60 * 24)); 
...
const getHoroscope = function(horoscopeSignAsNumber) {
    const daysFromStart = get_days_from_start();
    const horoscopeJson = require('./database/horoscopeDb.json');
    const horoscopeIndex = (startingPoint + daysFromStart) % numHoroscopeDays;
    return horoscopeJson[horoscopeIndex + (horoscopeSignAsNumber - 1) * numHoroscopeDays]["description"];
}
